...
describe('generateWorld', () => {
  ...
  it.each(SEEDS)('seed %i: places the configured counts on distinct tiles', (seed) => {
    ...
    for (const kind of ['merchant', 'bladeOil', 'forge', 'grave', 'jewelryBox', 'golem', 'cauldron', 'beehive', 'crystalBall', 'lookout', 'waypoint', 'fairy', 'wishingWell', 'tent', 'woodcutter'] as const) expect(count(kind)).toBe(POI_COUNTS[kind])
